import { Line, type FabricObject, Point, Circle, type FabricObjectProps, type ObjectEvents, type SerializedObjectProps } from 'fabric';
...
export function updateLinePosition(target: FabricObject<Partial<FabricObjectProps>, SerializedObjectProps, ObjectEvents>, isCreating?: boolean) {
  ...
  if (p.type === 'line') {
    ...
    if (pointIds && pointIds.length === 2) {
      ...
      const points = (p as Line).calcLinePoints();
      ...
      firstPoint.set({ left: point1.x - CIRCLE_RADIUS, top: point1.y - CIRCLE_RADIUS });
      secondPoint.set({ left: point2.x - CIRCLE_RADIUS, top: point2.y - CIRCLE_RADIUS });
